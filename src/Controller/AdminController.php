...
class AdminController extends AbstractController
{
    ...
    public function orderStatus(Order $order, Request $request): Response
    {
        $validStatuses = [
            ...
            Order::STATUS_READY, Order::STATUS_SHIPPED, Order::STATUS_DELIVERED, Order::STATUS_CANCELLED,
        ];
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        if ($dateFilter) {
            ...
            switch ($dateFilter) {
                case 'today':
                    $from = $now->setTime(0, 0, 0); $to = $now->setTime(23, 59, 59); break;
                case 'week':
                    $from = $now->modify('monday this week')->setTime(0, 0, 0); $to = $now->setTime(23, 59, 59); break;
                case 'month':
                    $from = $now->modify('first day of this month')->setTime(0, 0, 0); $to = $now->setTime(23, 59, 59); break;
                default:
                    $from = $to = null;
            }
            if (!empty($from) && !empty($to)) {
        ...
        $response = new StreamedResponse(function () use ($orders) {
            ...
            fputcsv($handle, ['Referencia', 'Fecha', 'Estado', 'Cliente', 'Email', 'Teléfono', 'Dirección', 'Total', 'Productos'], ';');
            foreach ($orders as $order) {
                $customer = $order->getCustomer();
                $items = [];
                foreach ($order->getLines() as $line) {
                    $items[] = $line->getProductArticle() . ' x' . $line->getQuantity();
                }
                fputcsv($handle, [
                    $order->getReference(),
        ...
        $response->headers->set('Content-Disposition', 'attachment; filename="pedidos_' . date('Ymd_His') . '.csv"');
        return $response;
    }
